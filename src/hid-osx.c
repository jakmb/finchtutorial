...
static unsigned short get_vendor_id(IOHIDDeviceRef device) {
    return (unsigned short)get_int_property(device, CFSTR(kIOHIDVendorIDKey));
}

static unsigned short get_product_id(IOHIDDeviceRef device) {
    return (unsigned short)get_int_property(device, CFSTR(kIOHIDProductIDKey));
}
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
    ...
    IOHIDDeviceRef *device_array = calloc((unsigned long)num_devices, sizeof(IOHIDDeviceRef));
...
#define BUF_LEN 256
        ...
        if ((vendor_id == 0x0 && product_id == 0x0) ||
            ...
            cur_dev->usage_page = (unsigned short)get_int_property(dev, CFSTR(kIOHIDPrimaryUsagePageKey));
            cur_dev->usage = (unsigned short)get_int_property(dev, CFSTR(kIOHIDPrimaryUsageKey));

            /* Fill out the record */
            cur_dev->next = NULL;
            len = (size_t)make_path(dev, cbuf, sizeof(cbuf));
            ...
            /* Release Number */
            cur_dev->release_number = (unsigned short)get_int_property(dev, CFSTR(kIOHIDVersionNumberKey));
...
static void hid_report_callback(void *context, IOReturn result, void *sender,
    ...
    rpt->data = calloc(1, (unsigned long)report_length);
    memcpy(rpt->data, report, report_length);
    rpt->len = (size_t)report_length;
    rpt->next = NULL;
...
hid_device * HID_API_EXPORT hid_open_path(const char *path) {
    ...
    for (i = 0; i < num_devices; i++) {
        ...
        len = (size_t)make_path(os_dev, cbuf, sizeof(cbuf));
        if (!strcmp(cbuf, path)) {
            ...
            if (ret == kIOReturnSuccess) {
                ...
                dev->input_report_buf = calloc((unsigned long)max_input_report_len, sizeof(uint8_t));
...
static int set_report(hid_device *dev, IOHIDReportType type, const unsigned char *data, size_t length) {
    ...
    if (!dev->disconnected) {
        res = IOHIDDeviceSetReport(dev->device_handle,
                                   type,
                                   data[0], /* Report ID*/
                                   data_to_send, (CFIndex)length_to_send);
